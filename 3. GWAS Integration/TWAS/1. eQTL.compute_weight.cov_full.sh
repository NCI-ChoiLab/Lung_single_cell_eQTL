...
LDREF="/data/Choi_lung/TTL/LDREF_Tensor"
...
cd /data/Choi_lung/TTL/FUSION/$Cell_type/compute_weight_full/
...
OUT_DIR="/data/Choi_lung/TTL/FUSION/"$Cell_type/"compute_weight_full/WEIGHTS"
mkdir $OUT_DIR
...
$PLINK --bfile $LDREF/$Cell_type/$Cell_type"_"$CHR --pheno ./tmp/$OUT.pheno --make-bed --out ./tmp/$OUT --keep ./tmp/$OUT.pheno --chr $CHR --from-bp $P0 --to-bp $P1 --extract $LDREF/$Cell_type/$Cell_type"_"$CHR.bim

# Process all samples together (for reference purposes only since this is mult-ethnic data)

FINAL_OUT="$OUT_DIR/$GNAME"
cd tmp
Rscript /data/Choi_lung/TTL/FUSION/fusion_twas-master/FUSION.compute_weights.R --bfile $OUT --tmp $OUT --covar /data/Choi_lung/TTL/FUSION/Covariates/$Cell_type'_full.txt'  --out $FINAL_OUT --verbose 0 --save_hsq --PATH_plink $PLINK --PATH_gcta $GCTA --PATH_gemma $GEMMA --models blup,lasso,top1,enet
